---
title: Installation
description: Install Stricture and set up architecture enforcement in your TypeScript project
---

import { Tabs, TabItem } from '@astrojs/starlight/components';
import { Card, CardGrid } from '@astrojs/starlight/components';

Get Stricture running in your TypeScript project in minutes.

## Prerequisites

Before installing Stricture, ensure your project has:

- **Node.js** 18.x or later
- **TypeScript** 4.5 or later
- **ESLint** 8.x or 9.x (flat config recommended)
- A TypeScript project with `tsconfig.json`

## Choose Your Architecture

Stricture supports two workflows:

<CardGrid>
  <Card title="🎯 Use a Preset" icon="star">
    **Recommended for most projects**

    Get started with proven architecture patterns:
    - Hexagonal (Ports & Adapters)
    - Layered (N-tier)
    - Clean Architecture
    - Next.js / NestJS specific

    [Browse all presets →](/presets/)
  </Card>
  <Card title="🔧 Custom Architecture" icon="setting">
    **For unique patterns**

    Define your own boundaries and rules for:
    - Company-specific patterns
    - Domain-specific constraints
    - Reusable internal presets

    [Create custom presets →](/guides/custom-presets/)
  </Card>
</CardGrid>

---

## Quick Setup (Recommended)

The fastest way to get started:

### Step 1: Install Packages

Choose a preset that matches your architecture:

<Tabs>
  <TabItem label="Hexagonal">
    ```bash
    npm install -D @stricture/eslint-plugin @stricture/hexagonal
    ```
  </TabItem>
  <TabItem label="Layered">
    ```bash
    npm install -D @stricture/eslint-plugin @stricture/layered
    ```
  </TabItem>
  <TabItem label="Clean">
    ```bash
    npm install -D @stricture/eslint-plugin @stricture/clean
    ```
  </TabItem>
  <TabItem label="Next.js">
    ```bash
    npm install -D @stricture/eslint-plugin @stricture/nextjs
    ```
  </TabItem>
</Tabs>

> **Note**: `@stricture/core` is installed automatically as a dependency.

### Step 2: Configure ESLint (One Line!)

Add Stricture to your ESLint config:

```js title="eslint.config.js"
import stricture from '@stricture/eslint-plugin'

export default [
  stricture.configs.hexagonal()  // That's it!
]
```

**No `.stricture/config.json` needed!** The preset is loaded automatically from `node_modules`.

:::tip[Different ESLint Config Format?]
Using `.eslintrc.js` or need advanced configuration? See the [ESLint Setup Guide](/guides/eslint-setup/).
:::

### Step 3: Verify Installation

Run ESLint to verify Stricture is working:

```bash
npx eslint .
```

If you have architecture violations, you'll see errors like:

```
src/domain/user.ts
  5:1  error  Import from 'infrastructure' to 'domain' is not allowed
             Domain cannot depend on infrastructure
             @stricture/enforce-boundaries
```

✅ **Stricture is now enforcing your architecture!**

---

## Alternative: CLI Setup

Let Stricture set everything up automatically:

```bash
npx stricture init
```

The CLI will:
- ✅ Detect your project framework (Next.js, NestJS, etc.)
- ✅ Recommend appropriate presets
- ✅ Configure ESLint automatically
- ✅ Install required dependencies

[View full CLI documentation →](/api/cli/)

---

## Customizing Your Configuration

Need to add ignore patterns or custom rules? Pass options to the config:

```js title="eslint.config.js"
import stricture from '@stricture/eslint-plugin'

export default [
  stricture.configs.hexagonal({
    ignorePatterns: ['**/*.test.ts', 'src/legacy/**'],
    rules: [
      {
        id: 'no-legacy-imports',
        from: { pattern: 'src/**' },
        to: { pattern: 'src/legacy/**' },
        allowed: false,
        message: 'No new code should depend on legacy modules'
      }
    ]
  })
]
```

For complex configurations with many custom rules, you can use a separate config file:

```js title="eslint.config.js"
export default [
  stricture.configs.recommended()  // Reads .stricture/config.json
]
```

```json title=".stricture/config.json"
{
  "preset": "@stricture/hexagonal",
  "boundaries": [ /* many boundaries */ ],
  "rules": [ /* many rules */ ]
}
```

Learn more in the [Configuration Files Guide](/configuration/config-file/).

---

## IDE Integration

### VS Code

ESLint violations appear inline with the [ESLint extension](https://marketplace.visualstudio.com/items?itemName=dbaeumer.vscode-eslint):

1. Install the ESLint extension
2. Restart VS Code
3. Architecture violations appear as red squiggly lines

### WebStorm / IntelliJ

ESLint integration is built-in:

1. Go to **Settings → Languages & Frameworks → JavaScript → Code Quality Tools → ESLint**
2. Enable **Automatic ESLint configuration**
3. Violations appear inline

---

## TypeScript Path Aliases (Optional)

If your project uses TypeScript path aliases, Stricture automatically resolves them using your `tsconfig.json`:

```json title="tsconfig.json"
{
  "compilerOptions": {
    "baseUrl": ".",
    "paths": {
      "@/domain/*": ["src/domain/*"],
      "@/infrastructure/*": ["src/infrastructure/*"]
    }
  }
}
```

No additional configuration needed.

---

## Monorepo Setup

For monorepos, configure Stricture per-package:

```
my-monorepo/
├── packages/
│   ├── api/
│   │   ├── eslint.config.js     # stricture.configs.hexagonal()
│   │   └── package.json
│   ├── web/
│   │   ├── eslint.config.js     # stricture.configs.nextjs()
│   │   └── package.json
│   └── shared/
│       └── package.json
└── package.json
```

Each package can use a different preset. See the [Monorepos Guide](/guides/monorepos/) for details.

---

## Troubleshooting

### ESLint not finding violations

**Solution**:
1. Verify the preset package is installed (`@stricture/hexagonal`, etc.)
2. Check ESLint config has `stricture.configs.xxx()`
3. Run `npx eslint --debug .` to see detailed output

### TypeScript paths not resolving

**Solution**:
1. Ensure `tsconfig.json` is in your project root
2. Verify `baseUrl` and `paths` are configured correctly
3. Restart your IDE/ESLint server

### Preset not found

**Solution**:
1. Install the preset: `npm install -D @stricture/hexagonal`
2. Verify it's in `devDependencies` in `package.json`
3. Run `npm install` again

For more troubleshooting help, see the [Troubleshooting Guide](/guides/troubleshooting/).

---

## Next Steps

<CardGrid>
  <Card title="Quick Start Tutorial" icon="rocket">
    Build a working example in 5 minutes.

    [Get started →](/getting-started/quick-start/)
  </Card>
  <Card title="Core Concepts" icon="open-book">
    Learn how boundaries and rules work.

    [Learn concepts →](/getting-started/concepts/)
  </Card>
  <Card title="ESLint Setup" icon="setting">
    Advanced ESLint configuration options.

    [Configure ESLint →](/guides/eslint-setup/)
  </Card>
  <Card title="Browse Presets" icon="puzzle">
    Explore all architecture patterns.

    [View presets →](/presets/)
  </Card>
</CardGrid>
