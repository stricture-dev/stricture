...
Install the ESLint plugin (all architecture presets are bundled):
...
> **Note**: All official architecture presets are bundled with the plugin. `@stricture/core` is automatically installed as a dependency.

:::tip[Different Architecture?]
All architecture presets are already bundled! Just use a different config:
...
**No `.stricture/config.json` needed!** The preset is automatically loaded from your `node_modules`.
