...
Install the ESLint plugin and the hexagonal preset:
...
Using a different pattern? Install a different preset:
- Layered: `@stricture/layered`
- Clean Architecture: `@stricture/clean`
- Next.js: `@stricture/nextjs`
...
That's it! The preset provides complete boundary and rule definitions.
...
Add Stricture to your ESLint configuration. The easiest way is to use the preset's recommended config:

<Tabs>
  <TabItem label="Recommended (Legacy Config)">
    ...
    This automatically enables the `@stricture/enforce-boundaries` rule.
  </TabItem>
  <TabItem label="Manual (Flat Config)">
    ```js title="eslint.config.js" ins={2,6-13}
    // eslint.config.js
  ...
  <TabItem label="Manual (Legacy .eslintrc.js)">
    ```js title=".eslintrc.js" ins={3,5-7}
    // .eslintrc.js
    module.exports = {
      plugins: ['@stricture'],
      rules: {
  ...
  <TabItem label="Manual (Legacy .eslintrc.json)">
    ```json title=".eslintrc.json" ins={3,5-7}
    {
      "plugins": ["@stricture"],
      "rules": {
        "@stricture/enforce-boundaries": "error"
...
:::tip[Easiest Setup]
Use the preset's recommended config (e.g., `plugin:@stricture/hexagonal`) for automatic rule configuration with legacy ESLint configs.
:::
