...
Install the ESLint plugin, which bundles all official presets:

```bash
npm install -D @stricture/eslint-plugin
```

The core package and all architecture presets are included automatically.
...
:::note
Rule `severity` only adds a `[WARNING]` prefix to messages or disables individual rules with `"off"`. To control ESLint error vs warning reporting, set the ESLint rule level to `'warn'` or `'error'` in your ESLint config.
:::
